...
final class RoundRobinConnection[F[_]: Monad](
    random: Random[F],
    connections: Vector[Connection[F]]
) extends Connection[F] {

  def getAdmin: Resource[F, Admin[F]] =
    Resource
      .eval(random.nextIntBounded(connections.length))
      .flatMap(i => connections(i).getAdmin)

  def getTable(tableName: HBaseClientTableName): Resource[F, Table[F]] =
    Resource
      .eval(random.nextIntBounded(connections.length))
      .flatMap(i => connections(i).getTable(tableName))
}
